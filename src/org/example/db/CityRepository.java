...
public class CityRepository {
    ...
    public void rewriteStackOrder(List<City> orderedBottomToTop) throws Exception {
        try (Connection c = database.getConnection()) {
            c.setAutoCommit(false);
            try (PreparedStatement ps = c.prepareStatement(
                    "UPDATE cities SET stack_order = ? WHERE id = ?")) {
                for (int i = 0; i < orderedBottomToTop.size(); i++) {
                    ps.setInt(1, i);
                    ps.setLong(2, orderedBottomToTop.get(i).getId());
                    ps.addBatch();
                }
                ps.executeBatch();
                c.commit();
